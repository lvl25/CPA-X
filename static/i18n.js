...
(function () {
    ...
    const T = {
        ...
        '百万Tokens': { en: 'M Tokens', vi: 'M Tokens' },
        '美元': { en: 'USD', vi: 'USD' },
        '成功': { en: 'Success', vi: 'Thành công' },
        '失败': { en: 'Failed', vi: 'Thất bại' },
        'Token 价格设置（输入/输出/缓存）': { en: 'Token Pricing (Input/Output/Cache)', vi: 'Giá Token (Nhập/Xuất/Bộ đệm)' },
        '输入': { en: 'Input', vi: 'Nhập' },
        '输出': { en: 'Output', vi: 'Xuất' },
        '缓存': { en: 'Cache', vi: 'Bộ đệm' },
        '保存价格': { en: 'Save Pricing', vi: 'Lưu giá' },
        '千万Tokens': { en: '10M Tokens', vi: 'M Tokens' },
        '亿Tokens': { en: '100M Tokens', vi: 'M Tokens' },
        '美元/百万Tokens': { en: 'USD/M Tokens', vi: 'USD/M Tokens' },
        '美元/千万Tokens': { en: 'USD/10M Tokens', vi: 'USD/M Tokens' },
        '美元/亿Tokens': { en: 'USD/100M Tokens', vi: 'USD/M Tokens' },
        '来源': { en: 'Source', vi: 'Nguồn' },
        '计价口径': { en: 'Pricing basis', vi: 'Đơn vị tính giá' },
        '手动': { en: 'Manual', vi: 'Thủ công' },
        '混合': { en: 'Mixed', vi: 'Hỗn hợp' },
        '手动值优先': { en: 'manual values take priority', vi: 'giá trị thủ công ưu tiên' },
    ...
    const PATTERNS = {
        ...
        vi: [
            ...
            [/^缓存[：:]/, () => 'Bộ đệm: '],
            // Pricing section
            [/^当前生效价格（(.+)）$/, (m) => `Giá hiện hành (${m[1]})`],
            [/^当前生效价格$/, () => 'Giá hiện hành'],
            [/^计价口径[：:](.+)（固定.*）$/, (m) => `Đơn vị tính giá: ${m[1]} (cố định)`],
            [/^计价口径[：:](.+)$/, (m) => `Đơn vị tính giá: ${m[1]}`],
            [/^来源[：:]手动（自动同步已关闭）$/, () => 'Nguồn: thủ công (đồng bộ tự động đã tắt)'],
            [/^来源[：:]手动（自动同步已开启）$/, () => 'Nguồn: thủ công (đồng bộ tự động đã bật)'],
            [/^来源[：:]混合（(.+)；手动值优先）$/, (m) => `Nguồn: hỗn hợp (${m[1]}; giá trị thủ công ưu tiên)`],
            [/^来源[：:](.+)$/, (m) => `Nguồn: ${m[1]}`],
            [/^费用按当前生效价格计算[：:](.+)$/, (m) => `Chi phí theo giá hiện hành: ${m[1]}`],
            // Token unit千万
            [/千万Tokens/, () => 'M Tokens'],
            [/百万Tokens/, () => 'M Tokens'],
            [/美元\/百万Tokens/, () => 'USD/M Tokens'],
            // System info
